...
#define MAX485_DE 4 // CHECKING --- nó là gì ?
...
void updateConfigFromJSON(const char *jsonStr)
{
  ...
  if (xSemaphoreTake(configMutex, pdMS_TO_TICKS(5000)) == pdTRUE)
  {
    ...
    for (int i = 0; i < totalGroups; i++)
    {
      ...
      JsonArray regs = s["regs"].as<JsonArray>(); // CHECKING --- nó là gì - xem demo
      int rSize = regs.size();
...
void loadConfig()
{
  ...
  if (savedSlavesJSON.length() > 10) // CHECKING --- tại sao lại > 10
  {
  ...
  while (1)
  {
    ...
    if (!setup_mode && totalGroups > 0)
    {
      ...
      for (int i = 0; i < totalGroups; i++)
      {
        ...
        bool is_id_lost = false; // CHECKING --- tại sao k dùng if else
        char alarmMsg[128] = "";
        ...
        if (!is_id_lost)
        {
          for (int j = 0; j < myGroups[i].regCount; j++)
          {
            ...
            if (myGroups[i].regs[j].type == 0) // CHECKING --- cần demo để biết được mỗi type là gì - nhắc nhở thêm comment
            {
...
bool connectGSM()
{
  ...
  if (!modem.init())
    return false;
  esp_task_wdt_reset(); // CHECKING --- vì sao lại cần gọi nhiều lần như này
...
void connectMQTT()
{
  ...
  if (mqtt.connected())
    return;
  else if (mqtt.connect("esp32_factory_client"))
  {
...
void taskNetwork(void *pvParameters)
{
  ...
  unsigned long last_wifi_recheck = 0;

  const unsigned long WIFI_WAIT_TIME = 30000;         // Đợi WiFi 30s trước khi bật SIM
  const unsigned long WIFI_RECHECK_INTERVAL = 300000; // 5 phút check lại WiFi khi đang dùng GSM
  const unsigned long MQTT_RETRY_DELAY = 5000;        // Đợi 5s nếu kết nối MQTT hụt
  ...
  while (1)
  {
    ...
    if (current_wifi_ok)
    {
      if (!is_modem_sleeping)
      {
        Serial.println("[NET] Đã có WiFi, tiến hành tắt SIM để hạ nhiệt...");
    ...
    else
    {
      if (!gsm_ok || is_modem_sleeping)
      {
        ...
        {
          Serial.println("[NET] WiFi không khả dụng, đang kích hoạt GSM...");
          ...
          if (gsm_ok)
          {
            ...
            Serial.println("[NET] Chuyển sang sử dụng mạng GSM.");
          }
          else
          {
            Serial.println("[NET] Khởi động GSM thất bại. Sẽ thử lại sau 10s...");
      ...
      else
      {
        ...
        {
          Serial.println("[NET] Kiểm tra lại sóng WiFi (đang dùng GSM)...");
          requestWiFiConnect();
          last_wifi_recheck = millis();
        }
      }
    ...
    if (current_wifi_ok || (gsm_ok && !is_modem_sleeping))
    {
      ...
      {
        Serial.println("[MQTT] Mất kết nối Server, đang thử kết nối lại...");
        esp_task_wdt_reset();
        ...
        if (!mqtt.connected())
        {
          Serial.println("[MQTT] Kết nối thất bại, sẽ thử lại sau 5s.");
  ...
  while (1)
  {
    if (!setup_mode && mqtt.connected())
    {
      ...
      if (totalGroups > 0 && (millis() - last_data_send > DATA_INTERVAL))
      {
        ...
        // int offset = snprintf(payload, sizeof(payload), "{\"time\":\"%s\",\"data\":[", timeStr);
